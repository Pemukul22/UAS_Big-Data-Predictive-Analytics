...
with col3:
    ...
    sns.heatmap(corr_matrix, annot=True, cmap='coolwarm', fmt=".3f", ax=ax1)
    st.pyplot(fig1)
...
with col4:
    ...
    ax2.scatter(y_test, y_pred, alpha=0.5, color='blue', label='Data Prediksi')
    ax2.plot([y_test.min(), y_test.max()], [y_test.min(), y_test.max()], 'r--', lw=2, label='Garis Ideal')
    ...
    sns.histplot(residuals, kde=True, color='purple', bins=30, ax=ax4)
