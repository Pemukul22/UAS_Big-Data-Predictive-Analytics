...
# Custom CSS untuk tampilan kartu, font, dan perbaikan kontras warna adaptif
st.markdown("""
<style>
...
with tab_simulasi:
    ...
    with col_output:
        ...
        with st.container(border=True):
            ...
            # Menggunakan warna biru terang (#2563EB) agar sangat kontras dan jelas di mode gelap maupun terang
            st.markdown(f"<h1 style='font-size: 2.8rem; color: #2563EB; font-weight: 700; margin: 0;'>${pred_close:,.2f}</h1>", unsafe_allow_html=True)
            
            # Badge delta custom dengan kontras tinggi
            color_delta = "#10B981" if delta_open >= 0 else "#EF4444"
            sign_delta = "+" if delta_open >= 0 else ""
            st.markdown(f"<p style='color: {color_delta}; font-weight: 600; font-size: 1.1rem; margin-top: 4px;'>{sign_delta}${delta_open:,.2f} dari harga Open</p>", unsafe_allow_html=True)
            ...
            if delta_open >= 0:
                ...
                st.write("Skenario ini memperlihatkan koreksi harian, di mana batas terendah (Low) menarik rata-rata penutupan lebih rendah dari harga pembukaan.")
...
with tab_diagnostik:
    ...
    with col_d2:
        ...
        st.caption("Catatan: Pada data time-series finansial harian (OHLC), skor VIF antar harga (Open, High, Low) umumnya sangat tinggi karena pergerakannya secara alami saling mengikuti (highly co-integrated).")
